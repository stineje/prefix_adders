...
# Perl script to generate Knowles prefix adder
...
# prefix tree
printf("\n// prefix tree\n");
printf("\t knowles prefix_tree(h, c, p, g, sum, cout);\n\n",$XBITS-1,$XBITS-1);


printf("endmodule\n\n");


printf("module knowles (h, c, p, g, sum, cout);\n");
printf("\t\n");
printf("\tinput [%s:0] p;\n",$XBITS);
printf("\tinput [%s:0] g;\n",$XBITS);
printf("\toutput [%s:1] h;\n",$XBITS);
...
# Generate the Knowles parallel-prefix tree
# -----------------------------
$llevel=ceil(log($XBITS)/log(2));
print "\t// parallel-prefix, Knowles\n";
for($x=0; $x<$llevel; $x++){	# last level has fan-out of 2 
	$d=2**($x+1);
	$dh=2**$x;		# half d
	#$dd=2**($x+2);	# double d
	...
	if($x==0){
		...
		for($y=1; $y<$XBITS ; $y++) {
			if ($y==1) {
				printf ("\trgry g_%s_%s (H_%s_%s, {g[%s],g[%s]});\n",$y+$d-2,$y-1,$y+$d-2,$y-1,
				$y+$d-2,$y-1);
			}
			else {
		...
		$i=1; # line counter
		for($y=$dh; $y<$XBITS ; $y+=2) {
			printf ("\tgrey g_%s_%s (H_%s_%s, {H_%s_%s,H_%s_%s}, I_%s_%s);\n",
					$y,0,$y,0,
					$y,$y-$dh+1,$y-$dh+1,0,$y,$y-$dh+1);
			printf ("\tgrey g_%s_%s (H_%s_%s, {H_%s_%s,H_%s_%s}, I_%s_%s);\n",
					$y+1,0,$y+1,0,
					$y+1,$y+1-$dh+1,$y-$dh+1,0,$y+1,$y+1-$dh+1);
			if($i%8==0){
				print "\n";
			}
			$i++;
		...
		for($y=$dh; $y<$XBITS ; $y++) {
			if ($y<$d) { # bit < 4,8,.... generate grey cells. otherwise, create black cells
				if ($y==$dh) {
					printf ("\tgrey g_%s_%s (H_%s_%s, {H_%s_%s,g[%s]}, I_%s_%s);\n",
					$y,0,$y,0,
					$y,1,0,$y,1);
				}
				else {
