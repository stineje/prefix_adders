...
# prefix tree
printf("\n// prefix tree\n");
printf("\t sklansky prefix_tree(c, p[%s:0], g[%s:0]);\n\n",$XBITS-1,$XBITS-1);
...
for($x=0; $x<ceil(log($XBITS)/log(2)); $x++){
	$d=2**($x+1);
	$dh=2**$x;
	printf("\t// Stage %s: Generates G/P pairs that span %s bits\n",$x+1,2**($x));
	if($x==0){
		$i=1; # line counter
		for($y=0; $y<$XBITS ; $y+=$d) {
			if ($y==0) {
		...
		for($y=0; $y<$XBITS ; $y+=$d) {
			for($z=$y+$dh; $z<$y+$d; $z++) {
				if ($z-$y-$dh+1==1) {
					if ($y==0) {
						printf ("\tgrey g_%s_%s (G_%s_%s, {g[%s],G_%s_%s}, p[%s]);\n",
						$z,$y,$z,$y,
						$z,$y+$dh-1,$y,$z);
					}
					else {
						printf ("\tblack b_%s_%s (G_%s_%s, P_%s_%s, {g[%s],G_%s_%s}, {p[%s],P_%s_%s});\n",
						$z,$y,$z,$y,$z,$y,
						$z,$y+$dh-1,$y,$z,$y+$dh-1,$y);
					}
				}
				else {
					if ($y==0) {
